...
export default function UserCart(props)
{
    ...
    const handleRemoveItem = (productId)=>{
        ...
        let userObj = {
          ...user,
          shoppingCart: [...array.filter((product) =>{
  ...
  return (
    <div style={{float: "right", width: matches?"500px":"300px",marginRight:"30px"}}>
    ...
    >
        ...
        {props.cart?props.cart.map((product, index) => {
          if(!set.has(product._id))
          {
            set.add(product._id)
            return(
        <div  key={index} style={{margin:"0 5px", flexDirection: matches?"row":"column" ,display:"flex", justifyContent:"space-between", gap:"10px"}}>
          <Product
            style={{flex : 1}}
          imageHeight = {100}
          key={index}
        ...
        <div style={{display:"flex", flex : 1,flexDirection:"column", justifyContent:"space-between", whiteSpace:"nowrap",overflowY:"auto"}}>
         ...
         <Button onClick={()=>{handleRemoveItem(product._id)}}>Remove</Button>
           ...
           <FormControl variant="standard" fullWidth style={{borderTop: "1px solid gray"}}>
       <InputLabel shrink htmlFor="bootstrap-input" style={{paddingTop:"10px"}}>
    ...
    <div style={{display:"flex", justifyContent: "space-between"}}>
        <p>Tax</p><p>${taxAmount.toFixed(2)}</p>
    </div>
    <div style={{display:"flex", justifyContent: "space-between"}}>
